...
/**
 * Shared Anthropic Messages API client. Raw HTTP (matches the codebase's
 * existing AI clients), prompt caching via cache_control on system blocks,
 * schema-forced JSON output, full usage capture incl. cache read/write, and
 * connection/transient retry.
 *
 * `messages` is passed straight to the API, so callers may include image
 * content blocks for vision. `$logContext` labels warnings per feature
 * (Issue Studio, Theme Wizard, …).
 *
 * Extracted from App\Services\IssueStudio\AnthropicGateway (T3 W0) so the
 * Theme Wizard and Issue Studio share one client. Issue Studio's gateway now
 * subclasses this with no behavior change.
 */
class AnthropicClient
{
    protected string $apiKey;
    protected string $logContext = 'AI';
    ...
    public function complete(string $model, array $systemBlocks, array $messages, int $maxTokens = 4096, ?array $jsonSchema = null): array
    {
        ...
        try {
            ...
            Log::warning("{$this->logContext}: Anthropic connection failure, retrying once", ['error' => $e->getMessage()]);
            sleep(3);
            try {
                $response = $this->post($payload);
            } catch (\Illuminate\Http\Client\ConnectionException $e) {
        ...
        if (!$response->successful()) {
            ...
            Log::warning("{$this->logContext}: Anthropic API error", ['status' => $response->status(), 'error' => $err]);
