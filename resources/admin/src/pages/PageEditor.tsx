...
export default function PageEditor() {
  ...
  if (error) {
    return <div className="flex items-center justify-center h-screen text-red-500">Failed to load page</div>;
  }
    ...
    <div className="flex flex-col h-screen bg-gray-100">
      <div className="flex items-center justify-between px-4 py-2 bg-white border-b border-gray-200 shrink-0">
        <div className="flex items-center gap-3">
          <button onClick={() => navigate(`/sites/${siteId}/pages`)} className="p-1.5 hover:bg-gray-100 rounded-md">
            <ArrowLeft size={18} />
          </button>
          <div>
            <h1 className="text-lg font-semibold">{page?.title ?? 'Page'}</h1>
            <span className="text-xs text-gray-500">/{page?.slug}</span>
          </div>
          <span className={`text-xs px-2 py-0.5 rounded-full ${page?.status === 'published' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
            {page?.status ?? 'draft'}
          </span>
