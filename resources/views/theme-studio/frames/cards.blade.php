<div style="padding:2rem;background:var(--semantic-color-background-canvas,#fff);">
    <h2 style="font-size:var(--semantic-font-size-xl,1.5rem);font-weight:700;margin-bottom:1.5rem;font-family:var(--semantic-font-family-display,inherit);color:var(--semantic-color-text-h2,var(--semantic-color-text-heading,#111));"
        @if($themeStudio ?? false) data-theme-tokens="semantic.color.text.h2,semantic.color.text.heading,semantic.font.family.display,semantic.font.size.xl" data-theme-element="heading.h2" data-theme-element-id="cards-h2" @endif>
        Card Grid
    </h2>
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:1.5rem;">
        ...
        <div style="background:var(--semantic-color-background-raised,#fff);border:1px solid var(--semantic-color-border-default,#e5e7eb);border-radius:var(--semantic-size-radius-lg,0.5rem);overflow:hidden;{{ $i === 1 ? 'box-shadow:var(--semantic-shadow-lg,0 10px 15px rgba(0,0,0,0.1));' : '' }}{{ $i === 2 ? 'border-width:2px;border-color:var(--semantic-color-border-strong,#333);' : '' }}"
            ...
            <div style="padding:{{ $i === 3 ? '0.75rem' : '1.25rem' }};">
                <h3 style="font-weight:600;margin-bottom:0.5rem;font-size:{{ $i === 3 ? '0.95rem' : '1.125rem' }};color:var(--semantic-color-text-h3,var(--semantic-color-text-heading,#111));font-family:var(--semantic-font-family-display,inherit);"
                    @if($themeStudio ?? false) data-theme-tokens="semantic.color.text.h3,semantic.color.text.heading,semantic.font.family.display" data-theme-element="card.title" data-theme-element-id="card-title-{{ $i }}" @endif>
                    {{ $title }}
                </h3>
                ...
                <a href="#" style="font-size:0.875rem;font-weight:600;"
                    @if($themeStudio ?? false) data-theme-tokens="semantic.color.text.link,semantic.text.decoration.link" data-theme-element="link" data-theme-element-id="card-link-{{ $i }}" @endif>
                    Read more →
                </a>
            </div>
        </div>
