...
<div class="contact-form-block {{ $__customClass }} {{ $__hideOn['scopeClass'] }}" style="position:relative;{{ $__sharedStyle }}" @if($__htmlId) id="{{ $__htmlId }}" @endif @if($__animAttr) data-animation="{{ $__animAttr }}" @endif @if(!empty($__adv['ariaLabel'])) aria-label="{{ $__adv['ariaLabel'] }}" @endif>
        ...
        @foreach($fields as $field)
            <div style="margin-bottom: 1rem;">
                <label style="display: block; font-size: 0.875rem; font-weight: 500; margin-bottom: 0.25rem;">
                    {{ $field['label'] }}@if(!empty($field['required'])) <span style="color: var(--color-danger,#ef4444);">*</span>@endif
                </label>
                @if($field['type'] === 'textarea')
                    <textarea name="{{ \Illuminate\Support\Str::slug($field['label']) }}" rows="4"{{ !empty($field['required']) ? ' required' : '' }}
                              style="width: 100%; padding: 0.5rem 0.75rem; border: 1px solid var(--input-border,#d1d5db); border-radius:var(--border-radius-sm,0.375rem); font-family: inherit;"></textarea>
                @else
                    <input type="{{ $field['type'] }}" name="{{ \Illuminate\Support\Str::slug($field['label']) }}"{{ !empty($field['required']) ? ' required' : '' }}
                           style="width: 100%; padding: 0.5rem 0.75rem; border: 1px solid var(--input-border,#d1d5db); border-radius:var(--border-radius-sm,0.375rem);">
                @endif
            </div>
        @endforeach
