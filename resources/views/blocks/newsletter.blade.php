...
<div style="{{ $isCard ? 'border:1px solid var(--color-border,#e2e8f0);border-radius:var(--border-radius-md,0.5rem);padding:2rem;text-align:center;' : '' }}{{ $isFull ? 'background:var(--color-bg-alt,#eff6ff);padding:2rem;text-align:center;border-radius:var(--border-radius-md,0.5rem);' : '' }}">
    ...
    <form action="{{ $endpoint }}" method="POST" style="display:flex;gap:0.5rem;{{ ($isCard || $isFull) ? 'justify-content:center;' : '' }}max-width:400px;{{ ($isCard || $isFull) ? 'margin:0 auto;' : '' }}">
        <input type="email" placeholder="Email" required style="flex:1;padding:0.5rem 0.75rem;border:1px solid var(--color-border-strong,#d1d5db);border-radius:var(--border-radius-sm,0.375rem);font-size:0.875rem;" />
        <button type="submit" style="background:var(--color-primary,#3b82f6);color:var(--color-text-inverse,#fff);padding:0.5rem 1.25rem;border:none;border-radius:var(--border-radius-sm,0.375rem);font-weight:500;cursor:pointer;">{{ $buttonText }}</button>
    </form>
</div>
