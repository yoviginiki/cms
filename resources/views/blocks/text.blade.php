...
@endphp
<div{!! sp_editable($__blockId ?? '', 'content', 'richtext') !!} class="prose" style="{{ $textAlign ? "text-align:{$textAlign};" : '' }}{{ $textColor ? "color:{$textColor};" : '' }}{{ $fontSize ? "font-size:{$fontSize};" : '' }}{{ $fontWeight ? "font-weight:{$fontWeight};" : '' }}{{ $fontStyle ? "font-style:{$fontStyle};" : '' }}{{ $lineHeight ? "line-height:{$lineHeight};" : '' }}{{ $letterSpacing ? "letter-spacing:{$letterSpacing};" : '' }}{{ $textShadow ? "text-shadow:{$textShadow};" : '' }}">
    {!! $data['content'] ?? '' !!}
</div>
