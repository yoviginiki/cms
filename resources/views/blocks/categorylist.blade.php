...
<div class="categorylist-block {{ $__customClass }} {{ $__hideOn['scopeClass'] }}" style="position:relative;{{ $__sharedStyle }}" @if($__htmlId) id="{{ $__htmlId }}" @endif @if($__animAttr) data-animation="{{ $__animAttr }}" @endif @if(!empty($__adv['ariaLabel'])) aria-label="{{ $__adv['ariaLabel'] }}" @endif>
...
@if($style === 'badges')
    <div style="display:flex;flex-wrap:wrap;gap:0.5rem;">
        @foreach($categories as $cat)
            <a href="{{ $cat['url'] ?? '#' }}" style="display:inline-flex;align-items:center;gap:4px;padding:0.375rem 0.75rem;background:var(--color-bg-alt,#f3f4f6);border-radius:9999px;font-size:0.875rem;color:inherit;text-decoration:none;">
                {{ $cat['name'] ?? '' }}
...
@else
    <ul style="list-style:none;padding:0;margin:0;">
        @foreach($categories as $cat)
            <li style="display:flex;justify-content:space-between;padding:0.375rem 0;border-bottom:1px solid var(--color-bg-alt,#f3f4f6);">
