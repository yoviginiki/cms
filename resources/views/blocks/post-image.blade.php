...
<div class="post-image-block {{ $__effectScope }} {{ $__customClass }} {{ $__hideOn['scopeClass'] }}" style="position:relative;{{ $__sharedStyle }}" @if($__htmlId) id="{{ $__htmlId }}" @endif @if($__animAttr) data-animation="{{ $__animAttr }}" @endif>
...
@endphp
@if($imageUrl)
    <img class="img-filtered" src="{{ $imageUrl }}" alt="{{ $post?->title ?? '' }}" loading="lazy"
        style="width:100%;height:auto;object-fit:{{ $objectFit }};{{ $borderRadius ? "border-radius:{$borderRadius};" : '' }}{{ $aspectRatio ? "aspect-ratio:{$aspectRatio};" : '' }}{{ $__imageFilter }}" />
@endif
