...
@php
    ...
    $maxW = BlockStyle::safeDim($data['max_width'] ?? '1200px') ?: '1200px';
    ...
    if ($widthMode === 'full') {
        ...
        // The mobile rule in critical CSS pads section CONTENT with !important,
        // which would inset a section the author explicitly set to full — the
        // marker class lets that rule exempt this one.
        $flushClass = 'section-flush';
    } elseif ($widthMode === 'wide') {
        $innerWrap = 'max-width:1440px;margin:0 auto;'; // wide container
    } else {
        $innerWrap = "max-width:{$maxW};margin:0 auto;"; // contained (default / legacy)
    }
    $id = $data['anchor_id'] ?? '';
...
<section class="section-block {{ $flushClass ?? '' }} {{ $__effectScope }} {{ $__customClass }} {{ $__hideOn['scopeClass'] }}" @if($id) id="{{ $id }}" @elseif($__htmlId) id="{{ $__htmlId }}" @endif style="padding-top:{{ $padTop }};padding-bottom:{{ $padBottom }};position:relative;{{ $fullBleed ?? '' }}{{ $__sharedStyle }}{{ $legacyStyle }}" @if($__animAttr) data-animation="{{ $__animAttr }}" @endif @if(!empty($__adv['ariaLabel'])) aria-label="{{ $__adv['ariaLabel'] }}" @endif @if(!empty($data['scene'])) data-scene="{{ $data['scene'] }}" @endif @if(!empty($data['experienceTransition'])) data-experience-transition="{{ $data['experienceTransition'] }}" @endif @if(!empty($data['experienceEnter'])) data-experience-enter="{{ $data['experienceEnter'] }}" @endif @if(!empty($data['experiencePin'])) data-experience-pin="true" @endif>
