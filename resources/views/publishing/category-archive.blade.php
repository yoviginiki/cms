...
<head>
    ...
    <meta name="description" content="{{ \Illuminate\Support\Str::limit($category->description ?: ($site->seo_defaults['description'] ?? 'Posts in ' . ($displayName ?? $category->name)), 160) }}">
        ...
        @foreach($posts as $post)
            <article class="archive-card{{ $loop->first ? ' archive-card--featured' : '' }}" style="margin:0;">
                ...
                <a href="{{ $post->url_path }}" class="archive-card__media" style="display:block;overflow:hidden;">
                    <img src="{{ $post->featured_image }}" alt="{{ $post->title }}" loading="{{ $loop->first ? 'eager" fetchpriority="high' : 'lazy' }}" style="width:100%;height:200px;object-fit:cover;display:block;">
                </a>
                @endif
                <div class="archive-card__body" style="padding-top:.6rem;">
                    @if($post->category)<a href="{{ $post->category->url_path }}" class="archive-card__cat" style="font-size:.85rem;display:inline-flex;align-items:center;min-height:32px;text-transform:uppercase;letter-spacing:.04em;color:var(--color-primary,#c0392b);text-decoration:none;font-weight:600;">{{ $post->category->name }}</a>@endif
                    ...
                    <time style="font-size:.8125rem;color:var(--color-text-muted,#9ca3af);" datetime="{{ $post->published_at?->toIso8601String() }}">{{ $post->published_at?->format('M j, Y') }}</time>
                </div>
            </article>
        @endforeach
        </div>
