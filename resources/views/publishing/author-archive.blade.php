...
<head>
    ...
    <title>{{ $author->name }} | {{ $site->name }}</title>
    <meta name="description" content="Posts by {{ $author->name }}">
    <link rel="canonical" href="{{ $baseUrl }}/blog/author/{{ \Illuminate\Support\Str::slug($author->name) }}">
    <meta property="og:title" content="{{ $author->name }} | {{ $site->name }}">
