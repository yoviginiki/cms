...
final class InlineEditServiceTest extends TestCase
{
    ...
    public function test_unknown_field_path_is_rejected_422(): void
    {
        ...
        $this->assertSame(422, $this->httpStatus(fn () => $this->svc->sanitizeField($block, 'bogus', 'x')));
    }

    public function test_reserved_key_is_rejected_422(): void
    {
        $block = new Block(['type' => 'heading', 'data' => ['text' => 'Hi']]);
        $this->assertSame(422, $this->httpStatus(fn () => $this->svc->sanitizeField($block, '__style', ['x' => 1])));
    }

    public function test_deep_dotted_path_is_rejected_422(): void
    {
        $block = new Block(['type' => 'heading', 'data' => ['text' => 'Hi']]);
        $this->assertSame(422, $this->httpStatus(fn () => $this->svc->sanitizeField($block, 'text.inner', 'x')));
    }
    ...
    public function test_schema_invalid_value_is_rejected_422(): void
    {
        ...
        $block = new Block(['type' => 'heading', 'data' => ['text' => 'Hi']]);
        $this->assertSame(422, $this->httpStatus(fn () => $this->svc->sanitizeField($block, 'level', 'h9')));
    }
    ...
    public function test_shared_entity_block_is_forbidden_403(): void
    {
        foreach (['slider_ref', 'global_ref', 'menu'] as $type) {
            ...
            $this->assertSame(403, $this->httpStatus(fn () => $this->svc->assertPatchable($block)), $type);
        }
    }
    ...
    public function test_normal_block_is_patchable(): void
    {
        ...
        $this->assertSame(0, $this->httpStatus(fn () => $this->svc->assertPatchable($block)));
    }
    ...
    public function test_hash_mismatch_is_conflict_409(): void
    {
        ...
        $this->assertSame(409, $this->httpStatus(fn () => $this->svc->assertHashMatches($block, 'stale-hash-from-session')));
    }
    ...
    public function test_matching_hash_passes(): void
    {
        ...
        $this->assertSame(0, $this->httpStatus(fn () => $this->svc->assertHashMatches($block, $hash)));
    }

    public function test_null_hash_skips_lock(): void
    {
        $block = new Block(['type' => 'heading', 'data' => ['text' => 'current']]);
        $this->assertSame(0, $this->httpStatus(fn () => $this->svc->assertHashMatches($block, null)));
    }
}
