...
|---|---|---|---|
| RenderMode + `sp_editable()` (Phase 1 + rollout) | Complete | 9/10 | 30 блока (20 flat + 10 NESTED, вкл. table[2D клетки], pricingtable/featurecomparison[2-level plans.*.features.*]); nested addressing поддържа items.*.field; останалите ~78 partial-а (предимно структурни/динамични/споделени — не са кандидати) |
