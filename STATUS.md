...
### G-Q2 — Advanced SQL mode (scoped views + restricted role + guard + Show-as-SQL)  🟡 CODE-COMPLETE, RUNTIME UNVERIFIED  (built 2026-07-14, feature/collections)
**Escape-attempt tests written FIRST (per spec):** `SqlModeSecurityTest` — 20 fixtures. Guard/parse layer (14, ALL PASSING): DML/DDL/DCL, statement chaining, the acceptance `SELECT … ; DELETE` appended-reject, catalog probing (pg_*, information_schema, GUC), real-table + cross-schema (cq_*) refs, function abuse (pg_sleep/lo_import/dblink/query_to_xml/set_config), quoting tricks (double-quote/dollar/E-string/EXPLAIN ANALYZE), unknown-view, and legit inner-FROM functions (extract/substring) + read-only CTEs allowed. Runtime layer (6: role execution, cross-tenant RLS backstop, statement timeout, row cap, draft invisibility) **SKIP — `cms_sql_guest` role not yet provisioned.**
...
**⚠ GATE before exposing SQL mode to users (G-Q3 UI):** provision `cms_sql_guest` (`docs/sql-mode-role-setup.sql`, superuser one-time) → `collections:rebuild-views` → re-run `SqlModeSecurityTest` so the 6 runtime-isolation tests execute and pass. Do NOT ship the SQL editor UI until they're green. Guard + view layers are verified; only the role-privilege/timeout runtime wall awaits the role.
