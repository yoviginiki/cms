...
| 9 | Asset pipeline (WebP/hashing) | full | yes (AssetVariantsTest 4) | pass | pending live backfill+republish | 🟡 YELLOW | 2026-07-13: generation code was repaired in remediation (f9ba405: `decodePath`, variant-aware publisher w/ hashed copies, logged catch) but never tested or backfilled — 0/21 prod assets still had no variants. Now: `regenerateVariants()` + `assets:generate-variants` backfill command + first empirical tests (real GD upload → 5 variants incl. real WebP (RIFF magic verified), backfill, publisher rewrite+copy of variant files). GREEN after prod backfill + republish verified live (WebP served). See §9 addendum. |
